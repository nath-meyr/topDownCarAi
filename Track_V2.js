class Track {
    ...
    static ROAD_WIDTH = 8;  // Changed from 4 to 8 to represent full width
    ...
    createStraightWalls(segment, currentPosition, currentAngle) {
        ...
        let perpAngle = angleRad;
        let offsetX = (Track.ROAD_WIDTH / 2) * cos(perpAngle);
        let offsetY = (Track.ROAD_WIDTH / 2) * sin(perpAngle);
    ...
    createCurveWalls(segment, currentPosition, angles) {
        ...
        let innerRadius = segment.radius - (Track.ROAD_WIDTH / 2);
        let outerRadius = segment.radius + (Track.ROAD_WIDTH / 2);
        ...
        for (let i = 0; i <= steps; i++) {
            ...
            let innerX = centerX + (directionMultiplier * innerRadius * cos(angle));
            let innerY = centerY + (directionMultiplier * innerRadius * sin(angle));
            let outerX = centerX + (directionMultiplier * outerRadius * cos(angle));
            let outerY = centerY + (directionMultiplier * outerRadius * sin(angle));
    ...
    createCheckpoint(position, angle) {
        ...
        this.createLineBody(position, perpendicularAngle, Track.CHECKPOINT_GROUP);
    ...
    createLineBody(position, angle = 0, group = Track.CHECKPOINT_GROUP) {
        const p2line = new p2.Body({
            ...
            angle: radians(angle),  // Set the angle for the line body
